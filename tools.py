...
def get_exchange_rate(from_date: datetime.date, to_date: datetime.date, from_currency: str, to_currency: str) -> pd.DataFrame:
    '''
    Returns a dataframe of exchange rates between two dates

    to_date is inclusive
    '''
    exchange_ticker = f'{from_currency}{to_currency}=X'

    rate = yf.download(exchange_ticker, from_date, to_date + datetime.timedelta(days=1), 
                       auto_adjust=True)['Close'][exchange_ticker]
    return rate
